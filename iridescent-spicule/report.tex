...
The fundamental localization problem can be formulated as:
\begin{equation}
\boxed{
\begin{aligned}
...
\textbf{Challenge}: This is a \emph{non-convex} optimization problem due to the Euclidean norm constraints. Direct minimization can get stuck in local minima.
...
We extend the formulation to handle outliers by introducing binary trust variables $b_{ij} \in \{0, 1\}$:
\begin{equation}
\boxed{
\begin{aligned}
\min_{\mathbf{x}_1, \ldots, \mathbf{x}_n, \mathbf{b}} \quad & \sum_{(i,j) \in \mathcal{E}} b_{ij} \left(\|\mathbf{x}_i - \mathbf{x}_j\| - d_{ij}\right)^2 + \lambda \sum_{(i,j) \in \mathcal{E}} (1 - b_{ij}) \\
...
where $\lambda > 0$ is a penalty parameter that controls the sparsity preference (preferring to trust measurements).
...
The relaxed SDP becomes:
\begin{equation}
\boxed{
\begin{aligned}
\min_{\mathbf{Z}} \quad & \sum_{(i,j) \in \mathcal{E}} \left(\mathbf{Z}_{ii} + \mathbf{Z}_{jj} - 2\mathbf{Z}_{ij} - d_{ij}^2\right)^2 \\
...
To handle outliers, we use binary variables $b_{ij} \in \{0, 1\}$ indicating whether measurement $(i,j)$ is trusted:

\begin{equation}
\boxed{
\begin{aligned}
\min_{\mathbf{x}, \mathbf{b}} \quad & \sum_{(i,j) \in \mathcal{E}} b_{ij} \left(\|\mathbf{x}_i - \mathbf{x}_j\| - d_{ij}\right)^2 + \lambda \sum_{(i,j)} (1 - b_{ij}) \\
\text{s.t.} \quad & b_{ij} \in \{0, 1\}, \quad \forall (i,j) \in \mathcal{E}
\end{aligned}
}
\label{eq:miqp}
